...
void boingboing(uint16_t iterations, int delay, unsigned char mode,
...
void effect_z_updown_move(unsigned char positions[64],
		unsigned char destinations[64], char axis) {
	int px;
	for (px = 0; px < 64; px++) {
...
void effect_smileyspin(int count, int delay, char bitmap) {
	unsigned char dybde[] = { 0, 1, 2, 3, 4, 5, 6, 7, 1, 1, 2, 3, 4, 5, 6, 6, 2,
			2, 3, 3, 4, 4, 5, 5, 3, 3, 3, 3, 4, 4, 4, 4 };
	int d = 0;
	int flip = 0;
	int x, y, off;
...
void effect_pathmove(unsigned char *path, int length) {
	...
	for (i = (length - 1); i >= 1; i--) {
		for (z = 0; z < 8; z++) {
			...
			state = getvoxel(((path[(i - 1)] >> 4) & 0x0f),
					(path[(i - 1)] & 0x0f), z);
